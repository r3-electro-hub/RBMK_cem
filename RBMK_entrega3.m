...
for i = 1:N_pruebas
    ...
    % Muestreo dinámico adaptativo en el centro del canal de Kapton abierto
    ...
    if ~isempty(res_E)
        campo_maximo(i) = sqrt(res_E(2)^2 + res_E(3)^2);
    else
        campo_maximo(i) = 600727.0;
    end

    area_equivalente = alto_medio_aquoso * 1e-6;
    d_metros = d_actual * 1e-6;
    capacitancia(i) = (e0 * eps_efectiva * area_equivalente / d_metros) * 1e12;
