...
capacitancia = zeros(1, N_pruebas);

factor_seguridad = zeros(1, N_pruebas);
...
for i = 1:N_pruebas
    ...
    ei_saveas('sim_temp.fee');

    fprintf('Procesando iteración %d de %d (d = %d um)...\n', i, N_pruebas, d_actual);
   ...
   % =========================================================================
    % 9. EXTRACCIÓN DE RESULTADOS FÍSICOS REALES (MÉTODO SIMULADO)
    % =========================================================================
    % Muestreo del Campo Eléctrico exacto en la zona crítica (Canal inferior)
    ...
    if ~isempty(res_E)
        campo_maximo(i) = sqrt(res_E(4)^2 + res_E(5)^2);
    else
        % Si el punto cae en un nodo inválido, buscamos un desfase seguro de 1 um
        res_E_respaldo = eo_getpointvalues(x_centro_dinamico + 1, y_centro_dinamico);
        if ~isempty(res_E_respaldo)
            campo_maximo(i) = sqrt(res_E_respaldo(4)^2 + res_E_respaldo(5)^2);
        else
            campo_maximo(i) = NaN; % Marcamos como error de lectura en lugar de falsear datos
        end
    end
    ...
    if ~isnan(campo_maximo(i)) && campo_maximo(i) > 0
        factor_seguridad(i) = E_ruptura / campo_maximo(i);
    else
        factor_seguridad(i) = NaN;
    end
